...
Escogeremos aleatoriamente 10 muestras de cada grupo de análisis.

### 1.2. Tipo de experimento

El tipo de experimento es un análisis de genes diferencialmente expresados de RNA-seq y Allele-Specific Expression.
...
El análisis se ha realizado en R-Studio siguiendo los pasos del estudio :
...
Para conseguir los datos de los exones de cada tránscrito uso la librería **EnsDb.Hsapiens.v75**. La función *exonBy* filtra los valores requerimos a partir de los nombres de las filas de **Datos**.
...
```{r pheatmap, echo=FALSE, message=FALSE, fig.align='center', fig.width=6, fig.height=4}
library("pheatmap")
...
```{r pca,echo=FALSE, fig.align='center', fig.width=6, fig.height=3}
library(ggplot2)
...
Realizamos un Counts-Plot para la comparación ELI vs NIT. Se observa que el gen más significativo tiene un counts más elevado en SFI para mujeres que para hombres.

```{r message=FALSE, fig.align='center', fig.width=5, fig.height=4, echo=FALSE}
...
```{r fig.align='center', fig.width=7, fig.height=6, echo=FALSE}
...
En este caso se representan los cambios en el log fold por sexo.
...
```{r, echo = FALSE}
kable(matrix(v_countNA,ncol =3), col.names = names(listOfSelected))
```
...
include_graphics("results/ClusterProfileGOcnetplot.SFIvsNIT.pdf")
```

\newpage

I la tabla correspondiente:
...
Los guardamos en la carpeta de resultados como results_sva_*comparación*.

```{r, echo=FALSE}
res1OrderedDF <- as.data.frame(res1Ordered)
write.csv(res1OrderedDF, file = "./results/results_sva_SFIvsNIT.csv")
res2OrderedDF <- as.data.frame(res2Ordered)
write.csv(res2OrderedDF, file = "./results/results_sva_ELIvsNIT.csv")
res3OrderedDF <- as.data.frame(res3Ordered)
write.csv(res3OrderedDF, file = "./results/results_sva_ELIvsSFI.csv")
```
...
for (i in 1:length(listOfSelected)){
  ...
  if (length(rownames(enrich.result@result)) != 0) {
  write.csv(as.data.frame(enrich.result), 
             file =paste0("./results/","SVAClusterProfileGO.Results.",comparison,".csv"), 
             row.names = FALSE)
    
 pdf(file=paste0("./results/","SVAClusterProfileGOBarplot.",comparison,".pdf"))
    print(barplot(enrich.result, showCategory = 10, font.size = 9, 
            title = paste0("ClusterProfiler GO Pathway Analysis for ", comparison,". Barplot")))
  dev.off()    
  ...
  pdf(file = paste0("./results/","SVAClusterProfileGOcnetplot.",comparison,".pdf"))
    print(cnetplot(edox, showCategory = 5, foldChange=geneList ))
  dev.off()  
  }
}
...
include_graphics("results/SVAClusterProfileGOcnetplot.ELIvsSFI.pdf")
```
...
Tab.react <- read.csv2(file.path("./results/SVAClusterProfileGO.Results.ELIvsSFI.csv"), 
                       sep = ",", header = TRUE)
...
* ClusterProfileGOBarplot.SFIvsNIT.pdf - Barplot de GO para la comparación SFIvsNIT.
* ClusterProfileGOcnetplot.SFIvsNIT.pdf - Cnetplot de GO para la comparación SFIvsNIT.
...
* results_sva_ELIvsNIT.csv - Resultados anotados para ELI vs NIT sin efecto batch oculto
* results_sva_ELIvsSFI.csv - Resultados anotados para ELI vs SFI sin efecto batch oculto
* results_sva_SFIvsNIT.csv - Resultados anotados para SFI vs NIT sin efecto batch oculto
* SVAClusterProfileGO.Results.ELIvsSFI.csv- Resultados de GO para la comparación ELIvsSFI sin efecto batch oculto.
* SVAClusterProfileGOBarplot.ELIvsSFI.pdf - Barplot de GO para la comparación ELIvsSFI sin efecto batch oculto.
* SVAClusterProfileGOcnetplot.ELIvsSFI.pdf - Cnetplot de GO para la comparación ELIvsSFI sin efecto batch oculto.
